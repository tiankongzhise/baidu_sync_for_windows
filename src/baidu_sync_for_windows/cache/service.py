...
class CacheService:
    def __init__(self, engine: Engine|None = None,service_tag:str|None = None):
    ...
    def get_cache_record(self, *args,**kwargs)->Any|None:
        ...
        if len(args) + len(kwargs) > 2:
            raise ValueError(f"Too many arguments, only one of cache_key or service_tag and cache_key is allowed, got {args} and {kwargs}")
        match args:
            case [cache_key]:
                cache_key = cache_key
            case [service_tag, cache_key]:
                ...
                cache_key = cache_key
            case _:
                raise ValueError(f"Too many arguments, only one of cache_key or service_tag and cache_key is allowed, got {args}")
        if kwargs:
        ...
        if not all([service_tag, cache_key]):
            raise ValueError(f"service_tag and cache_key are required, got service_tag: {service_tag} and cache_key: {cache_key}")
    ...
    @overload
    def set_cache_record(self, service_tag: str, cache_key: str, cache_value: Any)->None:...
    def set_cache_record(self, *args,**kwargs)->None:
        ...
        if len(args) + len(kwargs) > 3:
            raise ValueError(f"Too many arguments, only one of cache_key, service_tag and cache_key is allowed, got {args} and {kwargs}")
        match args:
        ...
        if not all([service_tag, cache_key, cache_value]):
            raise ValueError(f"service_tag, cache_key and cache_value are required, got service_tag: {service_tag}, cache_key: {cache_key} and cache_value: {cache_value}")
        with self.get_session() as session:
    ...
    def clear_cache_record(self, *args,**kwargs)->None:
        ...
        if len(args) + len(kwargs) > 1:
            raise ValueError(f"Too many arguments, only one of service_tag is allowed, got {args} and {kwargs}")
        match args:
            case [service_tag]:
                service_tag = service_tag
            case _:
                raise ValueError(f"Too many arguments, only one of service_tag is allowed, got {args}")
        if kwargs:
            if 'service_tag' in kwargs:
                service_tag = kwargs['service_tag']
        if service_tag is None:
