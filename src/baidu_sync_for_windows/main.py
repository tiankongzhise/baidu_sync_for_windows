...
from baidu_sync_for_windows.service import (
    scan_service,
    compress_service,
    verify_service,
    backup_service,
...
def sync_object_consumer(disk_space_coordinator: DiskSpaceCoordinator):
    import time

    logger.log(
        "SERVICE_INFO",
        "sync_object_consumer start,will sleep 10 seconds,to make sure the producer has created same data to backup,please wait...",
    )
    time.sleep(10)
...
def start_consumer(consumer_worker: Callable,disk_space_coordinator: DiskSpaceCoordinator):
    consumer_thread = threading.Thread(target=consumer_worker,args=(disk_space_coordinator,))
    consumer_thread.start()
    return consumer_thread


def start_producer(
    producer_worker: Callable,
...
def main():
    ...
    try:
        ...
        consumer_thread = start_consumer(sync_object_consumer,disk_space_coordinator)
