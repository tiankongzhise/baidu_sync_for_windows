...
RecordTypeClass: TypeAlias = type[RecordType]
...
class SourceRecord(ServiceBase):
    ...
    __table_args__ = (
        UniqueConstraint(
            "computer_unique_tag",
            "target_object_path",
            name="uix_computer_unique_tag_target_object_path",
        ),
    )
...
class HashRecord(ServiceBase):
    ...
    source: Mapped["SourceRecord"] = relationship(
        "SourceRecord", backref="hash_records", init=False
    )
    ...
    __table_args__ = (UniqueConstraint("source_id", name="uix_source_id"),)
...
class CompressRecord(ServiceBase):
    ...
    source_id: Mapped[int] = mapped_column(Integer, ForeignKey("source_record.id"))
    source: Mapped["SourceRecord"] = relationship(
        "SourceRecord", backref="compress_records", init=False
    )
...
class EncryptNameCompressRecord(ServiceBase):
    ...
    source_id: Mapped[int] = mapped_column(Integer, ForeignKey("source_record.id"))
    source: Mapped["SourceRecord"] = relationship(
        "SourceRecord", backref="encrypt_name_compress_records", init=False
    )
    encrypt_file_name: Mapped[str] = mapped_column(String(64))
    encrypt_name_compress_object_path: Mapped[str] = mapped_column(String(255))
    __table_args__ = (UniqueConstraint("source_id", name="uix_source_id"),)

    def __str__(self) -> str:
        return f"EncryptNameCompressRecord(id={self.id}, source_id={self.source_id}, encrypt_file_name={self.encrypt_file_name}, encrypt_name_compress_object_path={self.encrypt_name_compress_object_path}, created_at={self.created_time_to_local_time}, updated_at={self.updated_time_to_local_time}, latested_at={self.latested_time_to_local_time})"
...
class VerifyRecord(ServiceBase):
    ...
    source_id: Mapped[int] = mapped_column(Integer, ForeignKey("source_record.id"))
    source: Mapped["SourceRecord"] = relationship(
        "SourceRecord", backref="verify_records", init=False
    )
...
class EncryptNameVerifyRecord(ServiceBase):
    ...
    source_id: Mapped[int] = mapped_column(Integer, ForeignKey("source_record.id"))
    source: Mapped["SourceRecord"] = relationship(
        "SourceRecord", backref="encrypt_name_verify_records", init=False
    )
...
class BackupRecord(ServiceBase):
    ...
    source_id: Mapped[int] = mapped_column(Integer, ForeignKey("source_record.id"))
    source: Mapped["SourceRecord"] = relationship(
        "SourceRecord", backref="backup_records", init=False
    )
...
class EncryptNameBackupRecord(ServiceBase):
    ...
    source_id: Mapped[int] = mapped_column(Integer, ForeignKey("source_record.id"))
    source: Mapped["SourceRecord"] = relationship(
        "SourceRecord", backref="encrypt_name_backup_records", init=False
    )
